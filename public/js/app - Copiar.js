...
function renderPhotos() {
  ...
  box.innerHTML = state.photos.map((photo) => `
    <article class="photo-card">
      ...
      <img src="${photo.image}" alt="Foto viaje" />
...
function handleChecklistActions(e) {
  const check = e.target.closest("[data-check-id]");

  if (check) {
    const item = state.checklist.find((x) => x.id === check.dataset.checkId);
  ...
  function handleFlexiblePlanActions(e) {
...
function addChatMessage(e) {
  ...
  state.chat.push({
    ...
    me:
      author === "Familia Mesa - Muñoz",

    system:
      author === "Mensaje del viaje",
...
function handlePendingActions(e) {
  ...
  if (doneBtn) {
    const item = state.pending.find((x) => x.id === doneBtn.dataset.pendingDone);
    if (!item) return;
  ...
  if (favBtn) {
    const item = state.pending.find((x) => x.id === favBtn.dataset.pendingFav);
    if (!item) return;

    const exists = state.favorites.find((x) => x.title === item.title);
